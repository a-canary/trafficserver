...
TEST_CASE("DBRecord()", "[constructor]")
{
  ptr = new Derived();
  REQUIRE(ptr != nullptr);
}

TEST_CASE("~DBRecord", "[destructor]")
{
  delete ptr;
}

TEST_CASE("DBRecord Schema Reset", "[constructor] [destructor] ")
{
  ptr = new Derived();
...
TEST_CASE("DBRecord addBit", "")
{
  a = Derived::schema.addBit();
}

TEST_CASE("DBRecord Test bit", "[constructor] [destructor] ")
{
  ptr = new Derived();
  ptr->packBit(a, 1);
  CHECK(ptr->unpackBit(a) == 1);
}
/*
